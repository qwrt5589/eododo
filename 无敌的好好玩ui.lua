...
UIG.Parent = Open
      ...
      function window.Tab(window, name, icon)
        ...
        neonGradient.Parent = TabText
        neonGradient.Rotation = 90
        neonGradient.Enabled = true
        
        local neonScript = Instance.new("LocalScript", TabText)
        neonScript.Name = "NeonEffect"
        neonScript.Source = [[
            local label = script.Parent
            local gradient = label.UIGradient
            local ts = game:GetService("TweenService")
            local ti = TweenInfo.new(3, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, -1)
            local colors = {
            ...
            local tween = ts:Create(gradient, ti, {Offset = Vector2.new(1, 0)})
            tween:Play()
        ]]
...
---------------分割线-------------------------------分割线-------------------------------分割线----------------
        ...
        function tab.section(tab, name, TabVal)
          ...
          sectionNeon.Enabled = true
          
          local sectionScript = Instance.new("LocalScript", SectionText)
          sectionScript.Name = "SectionNeon"
          sectionScript.Source = [[
              local label = script.Parent
              local gradient = label.UIGradient
              local ts = game:GetService("TweenService")
              local ti = TweenInfo.new(2.5, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, -1)
              local colors = {
          ...
          function section.Button(section, text, callback)
            ...
            btnNeon.Parent = Btn
            btnNeon.Rotation = 90
            btnNeon.Enabled = true
            
            local btnScript = Instance.new("LocalScript", Btn)
            btnScript.Name = "BtnNeon"
            btnScript.Source = [[
                local button = script.Parent
                local gradient = button.UIGradient
                local ts = game:GetService("TweenService")
                local ti = TweenInfo.new(2, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, -1)
                local colors = {
        ...
        function section:Label(text)
          ...
          labelNeon.Enabled = true
          
          local labelScript = Instance.new("LocalScript", TextLabel)
          labelScript.Name = "LabelNeon"
          labelScript.Source = [[
              local label = script.Parent
              local gradient = label.UIGradient
              local ts = game:GetService("TweenService")
              local ti = TweenInfo.new(1.5, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, -1)
              local colors = {
          ...
          function section.Toggle(section, text, flag, enabled, callback)
            ...
            toggleNeon.Enabled = true
            
            local toggleScript = Instance.new("LocalScript", ToggleBtn)
            toggleScript.Name = "ToggleNeon"
            toggleScript.Source = [[
                local button = script.Parent
                local gradient = button.UIGradient
                local ts = game:GetService("TweenService")
                local ti = TweenInfo.new(2.2, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, -1)
                local colors = {
                    Color3.fromRGB(255, 150, 150),
                    Color3.fromRGB(255, 225, 150),
                    Color3.fromRGB(225, 255, 150),
                    Color3.fromRGB(150, 255, 150),
                    Color3.fromRGB(150, 255, 225),
                    Color3.fromRGB(150, 200, 255),
                    Color3.fromRGB(225, 150, 255),
                    Color3.fromRGB(255, 150, 225)
                }
                local sequence = ColorSequence.new(colors)
                gradient.Color = sequence
                local tween = ts:Create(gradient, ti, {Offset = Vector2.new(1, 0)})
...
-- ... 其余功能代码（Keybind、Textbox、Slider、Dropdown等）保持原样，只需在对应位置添加霓虹灯效果 ...
