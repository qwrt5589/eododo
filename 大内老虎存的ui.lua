...
-- 增强的果冻动画Tween函数
function JellyTween(obj, t, data, jellyFactor)
    jellyFactor = jellyFactor or 0.3 -- 果冻效果强度
    ...
    if jellyFactor > 0 then
        spawn(function()
            ...
            local overshootTween = services.TweenService:Create(
                obj,
                TweenInfo.new(t[1] * 0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                {
                    Size = UDim2.new(
                        originalSize.X.Scale, 
                        originalSize.X.Offset * (1 + jellyFactor * 0.1),
                        originalSize.Y.Scale,
                        originalSize.Y.Offset * (1 + jellyFactor * 0.1)
                    )
                }
            )
            overshootTween:Play()
            ...
            -- 回弹到正常大小
            local bounceBackTween = services.TweenService:Create(
                obj,
                TweenInfo.new(t[1] * 0.3, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out),
                {
                    Size = originalSize
                }
            )
...
-- 滑动的果冻效果
function JellyScroll(obj, direction, intensity)
    intensity = intensity or 0.8
    
    if direction == "up" then
        -- 向上滑动的果冻效果
        JellyTween(obj, {0.3, 'Elastic', 'Out'}, {
            Position = UDim2.new(
                obj.Position.X.Scale,
                obj.Position.X.Offset,
                obj.Position.Y.Scale - 0.02,
                obj.Position.Y.Offset
            )
        }, intensity)
        
        wait(0.1)
        JellyTween(obj, {0.4, 'Elastic', 'Out'}, {
...
-- 展开/收起的果冻效果
function JellyExpand(obj, targetSize, isExpanding)
    local originalSize = obj.Size
    ...
    if isExpanding then
        ...
        wait(0.2)
        JellyTween(obj, {0.5, 'Elastic', 'Out'}, {
            Size = targetSize
        }, 0.3)
    else
        -- 收起的果冻效果
        JellyTween(obj, {0.3, 'Back', 'Out'}, {
            Size = UDim2.new(
                targetSize.X.Scale,
                targetSize.X.Offset * 0.9,
                targetSize.Y.Scale,
                targetSize.Y.Offset
            )
        }, 0.4)
...
-- 按钮点击的果冻效果
function JellyClick(obj)
    local originalSize = obj.Size
    
    -- 点击时收缩
    JellyTween(obj, {0.1, 'Quad', 'Out'}, {
        Size = UDim2.new(
            originalSize.X.Scale,
    ...
    -- 最终恢复
    JellyTween(obj, {0.2, 'Elastic', 'Out'}, {
        Size = originalSize
    }, 0.3)
end
...
function Ripple(obj)
    spawn(function()
        ...
        JellyTween(Ripple, {.3, 'Linear', 'InOut'}, {Position = UDim2.new(-5.5, 0, -5.5, 0), Size = UDim2.new(12, 0, 12, 0)})
        wait(0.15)
        JellyTween(Ripple, {.3, 'Linear', 'InOut'}, {ImageTransparency = 1})
        wait(.3)
        Ripple:Destroy()
    end)
end
...
function switchTab(new)
  ...
  if old == nil then
    ...
    JellyTween(new[1], {0.2, 'Elastic', 'Out'}, {ImageTransparency = 0}, 0.3)
    JellyTween(new[1].TabText, {0.2, 'Elastic', 'Out'}, {TextTransparency = 0}, 0.3)
    return
  end
  ...
  JellyTween(new[2], {0.4, 'Elastic', 'Out'}, {
      Position = UDim2.new(0, 0, 0, 0),
      ImageTransparency = 0
  }, 0.5)
  
  JellyTween(old[1], {0.2, 'Quad', 'Out'}, {ImageTransparency = 0.2})
  JellyTween(new[1], {0.2, 'Elastic', 'Out'}, {ImageTransparency = 0}, 0.3)
  JellyTween(old[1].TabText, {0.2, 'Quad', 'Out'}, {TextTransparency = 0.2})
  JellyTween(new[1].TabText, {0.2, 'Elastic', 'Out'}, {TextTransparency = 0}, 0.3)
...
function drag(frame, hold)
    ...
    local function update(input)
        ...
        JellyTween(frame, {0.1, 'Elastic', 'Out'}, {
            Position = UDim2.new(
                startPos.X.Scale, 
    ...
    hold.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            -- 开始拖拽时的果冻效果
            JellyTween(frame, {0.1, 'Back', 'Out'}, {
                Size = UDim2.new(
                    frame.Size.X.Scale,
                    frame.Size.X.Offset * 0.98,
                    frame.Size.Y.Scale,
                    frame.Size.Y.Offset * 0.98
                )
            }, 0.3)
            ...
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    dragging = false
                    -- 结束拖拽时的果冻效果
                    JellyTween(frame, {0.3, 'Elastic', 'Out'}, {
                        Size = frame.Size
                    }, 0.4)
                end
            end)
...
-- 滑动的果冻滚动系统
function createJellyScroll(frame)
    ...
    frame:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
        ...
        if deltaTime > 0.01 then
            ...
            if math.abs(deltaPos) > 5 then
                isScrolling = true
                
                -- 滚动时的果冻效果
                if deltaPos > 0 then
                    -- 向下滚动
                    JellyScroll(frame, "down", 0.6)
                else
                    -- 向上滚动
                    JellyScroll(frame, "up", 0.6)
                end
            end
    ...
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseWheel then
            ...
            -- 滚轮滚动的果冻效果
            if direction > 0 then
                JellyScroll(frame, "up", 0.8)
            else
                JellyScroll(frame, "down", 0.8)
            end
            
            -- 应用滚动
            JellyTween(frame, {0.4, 'Elastic', 'Out'}, {
...
-- 侧边栏果冻效果
function toggleSideBarJelly(sideBar, tabBtns, scriptTitle, isExpanded)
    local sideBarWidth = 110
    local sideBarCollapsedWidth = 8
    
    if isExpanded then
        -- 展开的果冻效果
        JellyExpand(sideBar, UDim2.new(0, sideBarWidth, 0, sideBar.Size.Y.Offset), true)
        JellyExpand(tabBtns, UDim2.new(0, sideBarWidth, 0, tabBtns.Size.Y.Offset), true)
        JellyExpand(scriptTitle, UDim2.new(0, 102, 0, scriptTitle.Size.Y.Offset), true)
    else
        -- 收起的果冻效果
        JellyExpand(sideBar, UDim2.new(0, sideBarCollapsedWidth, 0, sideBar.Size.Y.Offset), false)
        JellyExpand(tabBtns, UDim2.new(0, sideBarCollapsedWidth, 0, tabBtns.Size.Y.Offset), false)
        JellyExpand(scriptTitle, UDim2.new(0, 0, 0, scriptTitle.Size.Y.Offset), false)
    end
end
function library.new(library, name, theme)
    ...
    function UiDestroy()
        -- 销毁时的果冻效果
        JellyTween(Main, {0.4, 'Elastic', 'In'}, {
    ...
    spawn(function()
        ...
        JellyTween(Main, {0.6, 'Elastic', 'Out'}, {
    ...
    sideBarToggleBtn.MouseButton1Click:Connect(function()
        ...
        toggleSideBarJelly(Side, TabBtns, ScriptTitle, sideBarExpanded)
    ...
    function toggleui()
        ...
        -- UI大小切换的果冻效果
        JellyTween(Main, {0.5, 'Elastic', 'Out'}, {
    ...
    -- 应用果冻滚动效果
    local tabBtnsJellyScroll = createJellyScroll(TabBtns)
    
    -- 添加滚动弹性反馈
    TabBtns:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
        local currentPos = TabBtns.CanvasPosition.Y
        local maxScroll = TabBtns.CanvasSize.Y.Offset - TabBtns.AbsoluteWindowSize.Y
        
        -- 边界弹性效果
        if currentPos < 0 then
            JellyTween(TabBtns, {0.5, 'Elastic', 'Out'}, {
...
Open.MouseEnter:Connect(function()
    JellyTween(Open, {0.3, 'Elastic', 'Out'}, {
...
Open.MouseLeave:Connect(function()
    JellyTween(Open, {0.4, 'Elastic', 'Out'}, {
...
function window.Tab(window, name, icon)
    ...
    -- 应用果冻滚动效果到Tab内容
    local tabContentJellyScroll = createJellyScroll(Tab)
    
    -- 添加滚动边界弹性
    Tab:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
        local currentPos = Tab.CanvasPosition.Y
        local maxScroll = Tab.CanvasSize.Y.Offset - Tab.AbsoluteWindowSize.Y
        
        if currentPos < 0 then
            JellyTween(Tab, {0.5, 'Elastic', 'Out'}, {
                CanvasPosition = Vector2.new(0, 0)
            }, 0.6)
        elseif currentPos > maxScroll then
            JellyTween(Tab, {0.5, 'Elastic', 'Out'}, {
    ...
    TabBtn.MouseEnter:Connect(function()
        JellyTween(TabIco, {0.3, 'Elastic', 'Out'}, {
            ImageTransparency = 0,
            Size = UDim2.new(0, 28, 0, 28)
        }, 0.4)
        JellyTween(TabText, {0.3, 'Elastic', 'Out'}, {
            TextTransparency = 0,
    ...
    function tab.section(tab, name, TabVal)
        ...
        Section.MouseEnter:Connect(function()
            JellyTween(Section, {0.3, 'Elastic', 'Out'}, {
        ...
        -- SectionToggle点击的果冻效果
        SectionToggle.MouseButton1Click:Connect(function()
            JellyClick(SectionToggle)
        end)
        ...
        -- 应用果冻滚动效果到Section内容
        local sectionJellyScroll = createJellyScroll(Objs)
        ...
        SectionToggle.MouseButton1Click:Connect(function()
            ...
            -- 展开的果冻效果
            JellyExpand(Section, UDim2.new(1, -20, 0, targetHeight), open)
            
            if open then
                wait(0.2)
                JellyExpand(Objs, UDim2.new(1, -12, 0, ObjsL.AbsoluteContentSize.Y + 8), true)
            else
                JellyExpand(Objs, UDim2.new(1, -12, 0, 0), false)
                wait(0.3)
                Objs.Visible = false
            end
            
            JellyTween(SectionOpened, {0.3, 'Elastic', 'Out'}, {
                ImageTransparency = (open and 0 or 1)
            }, 0.4)
            JellyTween(SectionOpen, {0.3, 'Elastic', 'Out'}, {
        ...
        ObjsL:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            ...
            JellyExpand(Objs, UDim2.new(1, -12, 0, contentHeight), true)
        end)
        ...
        function section.Button(section, text, callback)
            ...
            Btn.MouseEnter:Connect(function()
                JellyTween(Btn, {0.3, 'Elastic', 'Out'}, {
    ...
    spawn(function()
        wait(0.1)
        JellyTween(TextLabel, {0.4, 'Elastic', 'Out'}, {
            BackgroundTransparency = ALTransparency,
            TextTransparency = 0,
            Size = UDim2.new(1, 4, 0, 26)
        }, 0.5)
        wait(0.2)
        JellyTween(TextLabel, {0.3, 'Elastic', 'Out'}, {
...
function section.Toggle(section, text, flag, enabled, callback)
    ...
    ToggleBtn.MouseEnter:Connect(function()
        JellyTween(ToggleBtn, {0.3, 'Elastic', 'Out'}, {
            BackgroundTransparency = ALTransparency - 0.1,
            Size = UDim2.new(1, 4, 0, 42)
        }, 0.4)
        JellyTween(ToggleDisable, {0.3, 'Elastic', 'Out'}, {
            BackgroundColor3 = Color3.fromRGB(40, 40, 40),
    ...
    local funcs = {
        SetState = function(self, state)
            ...
            -- Toggle切换的果冻效果
            if state then
                JellyTween(ToggleSwitch, {0.4, 'Elastic', 'Out'}, {
            ...
            -- 添加脉冲果冻效果
            wait(0.2)
            JellyTween(ToggleSwitch, {0.3, 'Elastic', 'Out'}, {
...
function section.Keybind(section, text, default, callback)
    ...
    KeybindBtn.MouseEnter:Connect(function()
        JellyTween(KeybindBtn, {0.3, 'Elastic', 'Out'}, {
            BackgroundTransparency = ALTransparency - 0.1,
            Size = UDim2.new(1, 4, 0, 42)
        }, 0.4)
        JellyTween(KeybindValue, {0.3, 'Elastic', 'Out'}, {
            BackgroundColor3 = Color3.fromRGB(40, 40, 40),
            Size = UDim2.new(0, 106, 0, 32)
        }, 0.4)
    end)
    ...
    KeybindValue.MouseEnter:Connect(function()
        JellyTween(KeybindValue, {0.3, 'Elastic', 'Out'}, {
            ...
            Size = UDim2.new(0, KeybindValue.TextBounds.X + 36, 0, 32)
        }, 0.4)
    end)
    ...
    KeybindValue.MouseButton1Click:Connect(function()
        ...
        wait(0.1)
        JellyTween(KeybindValue, {0.2, 'Elastic', 'Out'}, {
            BackgroundColor3 = Color3.fromRGB(45, 45, 45),
        ...
        JellyTween(KeybindValue, {0.2, 'Elastic', 'Out'}, {
            ...
            Size = UDim2.new(0, KeybindValue.TextBounds.X + 40, 0, 36)
        }, 0.4)
...
function section.Textbox(section, text, flag, default, callback)
    ...
    TextboxBack.MouseEnter:Connect(function()
        JellyTween(TextboxBack, {0.3, 'Elastic', 'Out'}, {
            BackgroundTransparency = ALTransparency - 0.1,
            Size = UDim2.new(1, 4, 0, 42)
        }, 0.4)
        JellyTween(BoxBG, {0.3, 'Elastic', 'Out'}, {
            BackgroundColor3 = Color3.fromRGB(40, 40, 40),
            Size = UDim2.new(0, 106, 0, 32)
        }, 0.4)
    end)
    ...
    TextboxBack.MouseLeave:Connect(function()
        ...
        JellyTween(BoxBG, {0.3, 'Elastic', 'Out'}, {
            BackgroundColor3 = Background,
            Size = UDim2.new(0, TextBox.TextBounds.X + 30, 0, 28)
        }, 0.3)
    end)
    ...
    TextBox.Focused:Connect(function()
        JellyTween(BoxBG, {0.3, 'Elastic', 'Out'}, {
            ...
            Size = UDim2.new(0, TextBox.TextBounds.X + 36, 0, 34)
        }, 0.5)
        JellyTween(TextBox, {0.3, 'Elastic', 'Out'}, {
...
function section.Slider(section, text, flag, default, min, max, precise, callback)
    ...
    local funcs = {
        SetValue = function(self, value)
            ...
            -- 滑块移动的果冻效果
            JellyTween(SliderPart, {0.4, 'Elastic', 'Out'}, {
                Size = UDim2.new(percent, 0, 1, 0)
            }, 0.6)
            
            -- 值显示的果冻效果
            JellyTween(SliderValBG, {0.2, 'Elastic', 'Out'}, {
    ...
    SliderBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = true
            JellyTween(SliderBar, {0.3, 'Elastic', 'Out'}, {
                Size = UDim2.new(0, 144, 0, 16)
            }, 0.5)
            funcs:SetValue()
        end
    end)
    ...
    SliderValue.Focused:Connect(function()
        boxFocused = true
        JellyTween(SliderValBG, {0.3, 'Elastic', 'Out'}, {
            BackgroundColor3 = Color3.fromRGB(50, 50, 50),
            Size = UDim2.new(0, 50, 0, 32)
        }, 0.5)
    end)
    ...
    return funcs
            ...
            function section.Dropdown(section, text, flag, options, callback)
                ...
                DropdownTop.MouseEnter:Connect(function()
                    JellyTween(DropdownTop, {0.3, 'Elastic', 'Out'}, {
                        BackgroundTransparency = ALTransparency - 0.1,
                        Size = UDim2.new(1, 4, 0, 42)
                    }, 0.4)
                    JellyTween(DropdownOpen, {0.3, 'Elastic', 'Out'}, {
                        TextColor3 = Color3.fromRGB(160, 0, 255),
                        Size = UDim2.new(0, 24, 0, 24)
                    }, 0.4)
                end)
                ...
                local ToggleDropVis = function()
                    ...
                    if open then
                        ...
                        JellyExpand(DropdownModule, UDim2.new(1, 0, 0, targetHeight), true)
                    else
                        JellyExpand(DropdownModule, UDim2.new(1, 0, 0, 38), false)
                    end
                end
    
                DropdownOpen.MouseButton1Click:Connect(function()
                    JellyClick(DropdownOpen)
                    ToggleDropVis()
                end)
                ...
                funcs.AddOption = function(self, option)
                    ...
                    Option.MouseEnter:Connect(function()
                        JellyTween(Option, {0.3, 'Elastic', 'Out'}, {
                            BackgroundTransparency = ALTransparency - 0.1,
                    ...
                    if open then
                        ...
                        spawn(function()
                            wait(0.1)
                            JellyTween(Option, {0.4, 'Elastic', 'Out'}, {
                                BackgroundTransparency = ALTransparency,
                                TextTransparency = 0,
                                Size = UDim2.new(1, 4, 0, 30)
                            }, 0.5)
                ...
                funcs.RemoveOption = function(self, option)
                    ...
                    if option then 
                        JellyTween(option, {0.3, 'Elastic', 'In'}, {
                            BackgroundTransparency = 1,
                            TextTransparency = 1,
