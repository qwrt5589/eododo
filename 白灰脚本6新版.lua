...
YU_SHEN:Label("倒卖思全嘉")
YU_SHEN:Label("作者：孟浩然")
YU_SHEN:Label("QQ1693323219")
YU_SHEN:Button("复制孟浩然QQ号",function()
...
YU_SHEN:Button("传送到玩家旁边", function()
    ...
    if tp_player and tp_player.Character and tp_player.Character.HumanoidRootPart then
        ...
        Notify("孟浩然", "已经传送到玩家身边", "rbxassetid://", 5)
    else
        Notify("孟浩然", "无法传送 玩家已消失", "rbxassetid://", 5)
    end
end)
...
YU_SHEN:Toggle("锁定传送", 'LockTPToggle', false, function(state)
    ...
    if state and playernamedied ~= "" then
        ...
        Notify("孟浩然", "锁定传送已开启", "rbxassetid://", 5)
    else
        if connection then
            connection:Disconnect()
        end
        Notify("孟浩然", "锁定传送已关闭", "rbxassetid://", 5)
    end
end)
...
YU_SHEN:Toggle("循环把玩家传送过来", 'LoopTPToggle', false, function(state)
    ...
    if state and playernamedied ~= "" then
        ...
        Notify("孟浩然", "循环传送已开启", "rbxassetid://", 5)
    else
        if connection then
            connection:Disconnect()
        end
        Notify("孟浩然", "循环传送已关闭", "rbxassetid://", 5)
    end
end)
...
YU_SHEN:Toggle("查看玩家", 'SpectateToggle', false, function(state)
    if state then
        ...
        if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("Humanoid") then
            ...
            Notify("孟浩然", "已开启查看玩家", "rbxassetid://", 5)
        else
            Notify("孟浩然", "无法查看玩家", "rbxassetid://", 5)
        end
    else
        ...
        Notify("孟浩然", "已关闭查看玩家", "rbxassetid://", 5)
    end
end)
...
YU_SHEN:Button("甩飞一次选中的人", function()
    ...
    if not TargetPlayer or TargetPlayer == Player then
        Notify("孟浩然", "无玩家可甩飞", "rbxassetid://", 5)
        return
    end
    ...
    local SkidFling = function(Target)
        ...
        if not (Character and Humanoid and RootPart and TCharacter and THumanoid) then
            return Message("孟浩然", "玩家已趋势", 2)
        end
        if THumanoid.Sit then return Message("孟浩然", "目标处于坐姿", 2) end
        if not TCharacter:FindFirstChildWhichIsA("BasePart") then return Message("孟浩然", "玩家已趋势", 2) end
        ...
        Message("孟浩然", "已甩飞选中玩家", 2)
    end

    if TargetPlayer.UserId ~= 1414978355 then
        SkidFling(TargetPlayer)
    else
        Message("孟浩然", "该玩家存在甩飞名单", 2)
    end
end)
...
YU_SHEN:Toggle("循环甩飞", 'LoopFlingToggle', false, function(state)
    ...
    local function performFling()
        ...
        if not selectedPlayer or selectedPlayer == game.Players.LocalPlayer then
            Notify("孟浩然", "无玩家可甩飞", "rbxassetid://", 2)
            isRunning = false
            return
        end
        ...
        if THumanoid.Sit then
            Notify("孟浩然", "目标处于坐姿", "rbxassetid://", 2)
            isRunning = false
            return
        end
        if not TCharacter:FindFirstChildWhichIsA("BasePart") then
            Notify("孟浩然", "玩家已趋势", "rbxassetid://", 2)
            isRunning = false
            return
        end
    ...
    if state and playernamedied ~= "" then
        performFling()
        Notify("孟浩然", "循环甩飞已开启", "rbxassetid://", 2)
    else
        Notify("孟浩然", "循环甩飞已关闭", "rbxassetid://", 2)
    end
end)
...
YU_SHEN:Button("甩飞所有人", function()
    ...
    if not hasPlayers then
        Notify("孟浩然", "无玩家可以甩飞", "rbxassetid://", 2)
        return
    end
    ...
    local SkidFling = function(TargetPlayer)
        ...
        if Character and Humanoid and RootPart then
            ...
            elseif not TRootPart and not THead and Accessory and Handle then
                ...
                return Notify("孟浩然", "玩家已趋势", "rbxassetid://", 2)
            end
            ...
            return Notify("孟浩然", "随机错误", "rbxassetid://", 2)
        end
    end
    ...
    for _,x in next, Players:GetPlayers() do
        if x ~= Player then
            if x.UserId ~= 1414978355 then
                ...
                Notify("孟浩然", "玩家 " .. x.Name .. " 存在甩飞名单", "rbxassetid://", 2)
            end
        end
    end
    
    Notify("孟浩然", "已开始甩飞所有人", "rbxassetid://", 2)
end)
